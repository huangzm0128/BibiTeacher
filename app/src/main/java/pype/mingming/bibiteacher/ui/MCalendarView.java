...
/**
 * 自定义日历视图---自己画一个日历视图
 * Created by mingming on 2016/10/5.
 */
...
public class MCalendarView extends View {
    ...
    public MCalendarView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                //可以在这里添加已经选择的的日期
//                if(selectedDays != null && selectedDays[0] == y && selectedDays[1] == m){
//                    for(int k = 2; k<selectedDays.length; k ++){
//                        if(daysArray[i * 7 + j] == selectedDays[k]){
//                            canvas.drawCircle((width * j) + width / 2 + xOffset, (height * (i + 1)) + height / 2 + yOffset,
//                                    textSize * 1f, pointPaint);
//                        }
//                    }
//                }
                ...
                if(selectedDays != null ) {
                    for (int a = 0; a < selectedDays.size(); a++) {
                        if (selectedDays.get(a).getYear() == y && selectedDays.get(a).getMonth() == m
                                && selectedDays.get(a).getDay() == daysArray[i * 7 + j]) {
    ...
    public void setSelectedDays(List<MyDate> selectedDays){
        this.selectedDays = selectedDays;
    }

    public List<MyDate> getSelectedDays(){
        return selectedDays;
    }
}
